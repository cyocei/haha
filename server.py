...
def check_username():
    try:
        ...
        # Check if we were redirected to an error page or home page
        ...
        # More accurate existence check
        exists = False
        
        # If we have explicit existence strings, use them
        if e_string and e_string.lower() in response.text.lower():
            if m_string:
                # If we have both e_string and m_string, make sure m_string is NOT present
                exists = m_string.lower() not in response.text.lower()
            else:
                exists = True
        # If we only have missing strings, check those
        elif m_string and m_string.lower() not in response.text.lower():
            exists = True
        # Otherwise, use status codes and heuristics
        else:
            # Check if the status code matches expected codes
            if response.status_code == e_code:
                exists = True
                # Double check for common error strings
...
def get_metadata():
    try:
        with open('sites.json', 'r', encoding='utf-8') as f:
            data = json.load(f)
            # Only include sites that have all required fields and proper validation strings
